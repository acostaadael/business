              ...
              <div class="form-group">
                ...
                <select class="form-control" id="product-um" data-cy="um" name="um" v-model="product.um" required>
                  ...
                  <option
                    ...
                    :key="umOption.id"
                  >
                    {{ `${umOption.name} (${umOption.description})` }}
                  </option>
                </select>
  ...
  if (product.value.id) {
    productService()
      ...
      .then((param: any) => {
        ...
        previousState();
        alertService.showInfo(t$('businessApp.product.updated', { param: param.id }));
      })
      .catch((error: any) => {
        isSaving.value = false;
        alertService.showHttpError(error.response);
    ...
    productService()
      ...
      .then((param: any) => {
        isSaving.value = false;
        previousState();
        alertService.showSuccess(t$('businessApp.product.created', { param: param.id }).toString());
      })
      .catch((error: any) => {
        isSaving.value = false;
        alertService.showHttpError(error.response);
